...
timeIndex0 = 1:length(rssiToNode0);
figure('Name','rssi to Node 0');
plot(timeIndex0,rssiToNode0);
xlim([0 length(rssiToNode0)]);

timeIndex1 = 1:length(rssiToNode1);
figure('Name','rssi to Node 1');
plot(timeIndex1,rssiToNode1);
xlim([0 length(rssiToNode1)]);

timeIndex2 = 1:length(rssiToNode2);
figure('Name','rssi to Node 2');
plot(timeIndex2,rssiToNode2);
xlim([0 length(rssiToNode2)]);

timeIndex3 = 1:length(rssiToNode3);
figure('Name','rssi to Node 3');
plot(timeIndex3,rssiToNode3);
xlim([0 length(rssiToNode3)]);

timeIndex5 = 1:length(rssiToNode5);
figure('Name','rssi to Node 5');
plot(timeIndex5,rssiToNode5);
xlim([0 length(rssiToNode5)]);


figure('Name','rssi');
p1 = plot(timeIndex0,rssiToNode0,'r');hold on;
p2 = plot(timeIndex1,rssiToNode1,'b');hold on;
p3 = plot(timeIndex2,rssiToNode2,'m');hold on;
p4 = plot(timeIndex3,rssiToNode3,'k');hold on;
p5 = plot(timeIndex5,rssiToNode5,'c');
