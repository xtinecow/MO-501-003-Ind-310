...
namespace Indoor_Positioning_GUI
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void draw_ellipse(double lat, double longi, double latError, double longError, double angle)
        {
            ...
            {
                double theta = seg * i;      
                double x = lat + a * Math.Cos(theta) * Math.Cos(angle) - b * Math.Sin(theta) * Math.Sin(angle);
                double y = longi + b * Math.Sin(theta) * Math.Cos(angle) + a * Math.Cos(theta) * Math.Sin(angle);
        ...
        private void updateMarkerList()
        {
            ...
            for (int i = 0; i < marker_List.Count(); i++)
            {
                ...
                {
                    marker_List[i].update(latitude, longitude, latError, longError, errAngle);
                    checkUpdated = true;
            ...
            if (!checkUpdated)
            {
                MyMarker newMarker = new MyMarker(markerID, latitude, longitude, latError, longError, errAngle);
        ...
        private void drawMarkers()
        {
            ...
            foreach (MyMarker m in marker_List)
            {
                ...
                {
                    draw_ellipse(Convert.ToDouble(m.getLatitude()), Convert.ToDouble(m.getLongitude()), Convert.ToDouble(m.getXerror()), Convert.ToDouble(m.getYerror()), Convert.ToDouble(m.geterrAngle()));                    
                }
                if(!errorelipses_checkBox.Checked)
                {
                    polygons.Polygons.Clear();
        ...
        private MyMarker getCenter()
        {
            if (marker_List.Count() == 0)
            {
                MyMarker uofc = new MyMarker("center", "51.079948", "-114.125534", "0", "0", "0");
                return uofc;
            }
            ...
            MyMarker temp = new MyMarker("center", marker_List[0].getLatitude(), marker_List[0].getLongitude(), "0", "0","0");
            return temp;
        }
        ...
        public void startSocket()
        {
            try
            {
                ...
                IPAddress ipAddress = IPAddress.Parse("192.168.1.68");
    ...
    public class MyMarker
    {
        ...
        private string xerr;
        private string yerr;
        private string errAngle;

        public MyMarker(string ID, string lat, string longi, string xerr, string yerr, string aErr)
        {
            this.markerID = ID;
            this.lat = lat;
            this.longi = longi;
        ...
        public void update(string lat, string longi, string xerr, string yerr, string errAng)
        {
            setLatitude(lat);
            setLongitude(longi);
            setXerror(xerr);
        ...
        public string geterrAngle()
        {
            return errAngle;
        }

        public void seterrAngle(string eAng)
        {
            this.errAngle = eAng;
        }
